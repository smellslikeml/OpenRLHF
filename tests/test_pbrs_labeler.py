...
The pure labeler-logic tests load ``pbrs_utils.py`` directly via importlib
...
def _load_labeler_module():
    ...
    utils_dir = root / "openrlhf" / "utils"

    pkg = types.ModuleType(_TEST_PACKAGE)
    pkg.__path__ = [str(utils_dir)]
    sys.modules[_TEST_PACKAGE] = pkg

    spec = importlib.util.spec_from_file_location(f"{_TEST_PACKAGE}.pbrs_utils", utils_dir / "pbrs_utils.py")
    module = importlib.util.module_from_spec(spec)
    sys.modules[f"{_TEST_PACKAGE}.pbrs_utils"] = module
...
class TestPBRSLabelerBlend:
    """Blend validation, blend=0 no-op, base score NotImplementedError,
    error propagation, and zero-labeler blend cases."""

    def test_blend_must_be_in_unit_interval(self):
        with pytest.raises(AssertionError):
            PBRSLabeler(blend=1.5)
        with pytest.raises(AssertionError):
            PBRSLabeler(blend=-0.1)

    def test_blend_zero_is_noop_without_scoring(self):
    ...
    def test_base_score_is_not_implemented(self):
        with pytest.raises(NotImplementedError):
            PBRSLabeler(blend=1.0).apply([2.0], ["a"])

    def test_labeler_error_propagates(self):
        with pytest.raises(RuntimeError, match="labeler exploded"):
            _BoomLabeler(blend=1.0).apply([2.0], ["a"])

    def test_zero_labeler_full_blend_zeros_rewards(self):
        rewards = [2.0, 4.0]
        out = _ZeroLabeler(blend=1.0).apply(rewards, ["a", "b"])
        assert out == [0.0, 0.0]

    def test_zero_labeler_half_blend_halves_rewards(self):
        rewards = [2.0, 4.0]
        out = _ZeroLabeler(blend=0.5).apply(rewards, ["a", "b"])
        assert out == [1.0, 2.0]


class TestIdentityLabeler:
    """The shipped reference labeler is a no-op at every blend."""

    def test_identity_labeler_is_noop_at_any_blend(self):
        rewards = [2.0, 4.0]
...
class TestRemoteExperienceMakerWiring:
    """The reward-collection call site routes through _apply_pbrs_labeler."""

    def test_remote_experience_maker_wires_pbrs_labeler(self):

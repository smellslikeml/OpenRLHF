...
def test_disabled_is_noop():
    ...
    assert torch.equal(exp.advantages, before)


def test_high_surprisal_positive_advantage_is_boosted():
    args = _Args(
    ...
    assert exp.advantages[0, 0].item() == 2.0
    assert exp.advantages[0, 1].item() == 1.0
    assert exp.advantages[0, 3].item() == 1.0
    # Diagnostics are attached for logging.
    assert "surprisal_critical_frac" in exp.info


def test_sign_aware_leaves_negative_advantages_untouched():
    args = _Args(
    ...
    assert exp.advantages[0, 0].item() == -1.0
    assert exp.advantages[0, 1].item() == -1.0


def test_masked_tokens_are_ignored_for_quantile_and_reweight():
    args = _Args(
    ...
    surprisal_reweight.apply_surprisal_reweighting([exp], args)
    assert exp.advantages[0, 0].item() == 1.0  # masked token untouched


def test_call_site_invokes_reweighting():
    ...
    assert "apply_surprisal_reweighting" in source
    assert "from openrlhf.trainer.ppo_utils.surprisal_reweight import apply_surprisal_reweighting" in source
